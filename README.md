...
Instead the `.add_jobs(...)` and `.add_sequences(...)` methods of sequences accept a `namespace` parameter,
...
to the `.add_jobs(...)` and `.add_sequences(...)` method you cann pass the `name` parameter.
